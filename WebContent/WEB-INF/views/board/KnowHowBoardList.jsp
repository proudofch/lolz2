				...
				<div id="pagination_wrapper">
					<div class="pagination">
						<c:if test="${currentpage > 1}">
							<a href="KnowHowBoardList.Board?cp=${currentpage-1}&ps=${pagesize}"><i class="fas fa-angle-double-left"></i></a>
						</c:if>
						...
						<c:forEach var="i" begin="1" end="${pagecount}" step="1">
							<c:choose>
								...
								<c:otherwise>
									<a href="KnowHowBoardList.Board?cp=${i}&ps=${pagesize}">${i}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						
						<c:if test="${currentpage < pagecount}">
							<a href="KnowHowBoardList.Board?cp=${currentpage+1}&ps=${pagesize}"><i class="fas fa-angle-double-right"></i></a>
						</c:if>
					</div>
				</div>
				
				<c:if test="${sessionScope.id != null }">
					<a href="KnowHowBoardWrite.Board" class="button primary" id="btn">글쓰기</a>
				</c:if>
